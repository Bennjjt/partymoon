  ...
  return (
    ...
    >
      <div className="px-6 md:px-12 py-24">
        <RevealOnScroll>
          <p className="text-[0.6rem] tracking-[0.5em] uppercase mb-4" style={{ color: '#FF2D78' }}>All Weekend, Both Nights</p>
            ...
            <RevealOnScroll key={host.id ?? i} delay={i * 0.1}>
              <div className="p-10 relative overflow-hidden" style={{ background: 'var(--pm-deep)' }}>
                ...
                <div
                  ...
                  style={{ background: 'radial-gradient(circle, rgba(107,95,204,0.12) 0%, transparent 70%)' }}
                />

                {host.icon && (
                  <span className="block text-4xl mb-6" style={{ filter: 'drop-shadow(0 0 10px rgba(107,95,204,0.5))' }}>
                    {host.icon}
                  </span>
                )}
                {host.role && (
                  <p className="text-[0.6rem] tracking-[0.4em] uppercase font-medium mb-2" style={{ color: '#FF2D78' }}>
                    {host.role}
                  </p>
                )}
                {host.name && (
        ...
        <RevealOnScroll delay={0.2}>
          <div className="mt-8 p-10 text-center" style={{ background: 'rgba(107,95,204,0.06)', border: '1px solid rgba(107,95,204,0.15)' }}>
